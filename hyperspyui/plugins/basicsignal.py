...
class BasicSignalPlugin(Plugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.settings.set_default("histogram_bins_method", "auto")
    ...
    def stack(self, signals=None, advanced=False):
        ...
        if advanced:
            ...
            self.record_code('s_stack = hs.stack(signals, axis="%s")' % axis.name)
        else:
    ...
    def switch(self, signal=None):
        ...
        switched = signal.transpose(
            signal.axes_manager.navigation_axes, signal.axes_manager.signal_axes
        )
        switched.plot()
    ...
    def histogram(self, signal=None):
        ...
        if signal is not None:
            ...
            self.record_code("signal = ui.get_selected_signal()")
            self.record_code("histogram = signal.get_histogram(bins=%s)" % method)
    ...
    def add(self, signals=None):
        if signals is None:
            signals = self.ui.get_selected_signals()
        if len(signals) < 2:
            raise ValueError("Two or more signals should be selected for addition!")
