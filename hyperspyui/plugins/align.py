...
class AlignPlugin(Plugin):
    ...
    def create_actions(self):
        self.add_action(
            ...
            selection_callback=SignalTypeFilter(hs.signals.Signal2D, self.ui),
        )
    ...
    def align_XD(self, roi, signal=None):
        ...
        if roi.ndim == 1:
            ...
            raise ValueError("Cannot align a signal of %d dimensions" % roi.ndim)
    ...
    def align_1D(self, roi, signal=None):
        ...
        shifts = signal.estimate_shift1D(
            reference="current", roi=(roi.left, roi.right), show_progressbar=True
        )
        ...
        self.record_code(
            "s_aligned.align1D(reference='current', "
            "roi=(%f, %f), show_progressbar=True, expand=True)" % (roi.left, roi.right)
        )
        return s_aligned
    ...
    def align_2D(self, roi, signal=None):
        ...
        record_string = (
            ...
            ref, sobel, hanning, median, roi.left, roi.right, roi.top, roi.bottom, plot
        )
        try:
    ...
    def _align_along_axis(self, roi, signal, axis):
        ...
        d = np.array(
            [self._smooth(s_al.data[i, :], smooth) for i in range(s_al.data.shape[0])]
        )
...
class ManualAlignDialog(ExToolWindow):
    ...
    def update_x(self):
        ...
        if self.shifts is None:
            self.shifts = np.zeros((signal.axes_manager.navigation_size, 2), dtype=int)
        index = np.ravel_multi_index(
            signal.axes_manager.indices, signal.axes_manager.navigation_shape
        )
        with signal.unfolded(unfold_signal=False):
            axis = signal.axes_manager.signal_axes[0].index_in_array
            signal.data[index:, ...] = np.roll(signal.data[index:, ...], val, axis)
