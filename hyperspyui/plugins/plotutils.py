...
class PlotUtils(Plugin):
    ...
    def create_menu(self):
        self.add_menuitem("Image", self.ui.actions[self.name + ".plot_images"])
        self.add_menuitem("Spectrum", self.ui.actions[self.name + ".plot_spectra"])
    ...
    def plot_images(self, images=None):
        ...
        for im in images:
            if len(im.axes_manager.shape) != 2 and len(images) > 1:
                raise ValueError("Signal shape invalid for plot_images(): %s" % str(im))
            if im.axes_manager.signal_dimension == 2:
                images_t.append(im)
            elif len(im.axes_manager.shape) != 2:
                raise ValueError("Signal needs to be an image, or a stack of images")
            else:
