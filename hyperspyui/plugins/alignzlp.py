...
class Alignzlp(Plugin):
    ...
    def create_toolbars(self):
        self.add_toolbar_button("EELS", self.ui.actions[self.name + ".default"])
    ...
    def default(self):
        ...
        signal_list = [
            sig
            for sig in ui.signals
            if (
                sig.signal is not s
                and isinstance(sig.signal, exspy.signals.EELSSpectrum)
            )
        ]
        also_align = []
        if len(signal_list) > 0:
            picker = SignalList(items=signal_list, parent=ui, multiselect=True)
            diag = ui.show_okcancel_dialog(
                "Select signals to align with {}".format(title), picker, modal=True
            )
