...
class GaussianFilter(Plugin):
    ...
    def create_toolbars(self):
        self.add_toolbar_button("Image", self.ui.actions[self.name + ".show_dialog"])
    ...
    def selection_rules(self, win, action):
        ...
        ok = False
        if s is not None:
            if win == s.signal_plot and s.signal.axes_manager.signal_dimension == 2:
                ok = True
        if ok:
    ...
    def gaussian(self, sigma, signal=None, out=None, record=True, *args, **kwargs):
        """
        ...
        **kwargs : dict
        ...
        if np.issubdtype(signal.data.dtype, float):
            ...
            vmin, vmax = (None, None)
        for im_o, im_i in zip(s_out._iterate_signal(), signal._iterate_signal()):
            if vmin is None:
                im_o[:] = gaussian(im_i, sigma, *args, **kwargs)
            else:
                im_o[:] = gaussian(
                    (im_i - vmin) / (vmax - vmin), sigma, *args, **kwargs
                )
        if vmin is not None:
        ...
        if out is None:
            if record:
                self.record_code(
                    r"<p>.gaussian({0}, {1}, {2})".format(sigma, args, kwargs)
                )
            return s_out
        else:
            if record:
...
class GaussianFilterDialog(ExToolWindow):
    ...
    def ok(self):
        ...
        if (
            ...
            or not self.chk_preview.isChecked()
        ):
            self.update()
    ...
    def update(self):
        ...
        s = self.plugin.gaussian(sigma, self.signal.signal, record=False, out=out)
