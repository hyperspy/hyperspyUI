...
class MainWindow(MainWindowHyperspy):
    ...
    def __init__(self, splash=None, parent=None, argv=None):
        ...
        self.setWindowIcon(
            QtGui.QIcon(
                os.path.join(os.path.dirname(__file__), "images", "hyperspy.svg")
            )
        )
    ...
    def _load_signal_types(self):
        ...
        self.signal_types = set(
            [
                value["signal_type"]
                for value in ALL_EXTENSIONS["signals"].values()
    ...
    def parse_args(self, argv=None):
        ...
        parser.add_argument(
            "files", metavar="file", type=str, nargs="*", help="data file to open."
        )
        if argv is None:
    ...
    def create_default_actions(self):
        ...
        close_all_key = QtGui.QKeySequence(
            Qt.CTRL | Qt.ALT | Qt.Key_F4, Qt.CTRL | Qt.ALT | Qt.Key_W
        )
        self.add_action(
            "close_all",
            "&Close All",
            self.close_all_signals,
        ...
        self.add_action(
            "save_fig", "Save &figure", self.save_figure, tip="Save the active figure"
        )
    ...
    def select_x_signals(self, x, titles=None, wrap_col=4):
        ...
        """
        w = PickXSignalsWidget(
            self.signals, x, parent=self, titles=titles, wrap_col=wrap_col
        )
    ...
    def show_plugin_manager(self):
        if self._plugin_manager_widget is None:
            self._plugin_manager_widget = PluginManagerWidget(self.plugin_manager, self)
    ...
    def set_signal_dtype(self, data_type, signal=None, clip=False):
        ...
        if not clip:
            ...
            if type_info and old_type_info and type_info.max < old_type_info.max:
                ...
                self.record_code(
                    "signal.data *= %f / np.nanmax(signal.data)" % float(type_info.max)
                )
